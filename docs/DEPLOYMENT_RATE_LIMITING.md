...
⚠️ **Используйте только если** приложение открыто напрямую в интернет без nginx/прокси  
⚠️ **Не рекомендуется** - лучше поставить nginx или Cloudflare перед приложением  
⚠️ При запуске увидите предупреждение в логах
